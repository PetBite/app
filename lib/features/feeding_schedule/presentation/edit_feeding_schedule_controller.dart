...
class EditFeedingScheduleController extends _$EditFeedingScheduleController {
  ...
  Future<void> updateFeedingSchedule({
    ...
    final newState = await AsyncValue.guard(
        () => database.setFeedingSchedule(schedule, userId, petId));
    if (mounted) {
      state = newState;
    }
    if (!state.hasError) {
  ...
  Future<void> deleteFeedingSchedule({
    ...
    FeedingScheduleDatabase database =
        ref.watch(feedingScheduleDatabaseProvider);
    final newState = await AsyncValue.guard(
        () => database.deleteFeedingSchedule(schedule, userId, petId));
    if (mounted) {
      state = newState;
    }
    if (!state.hasError) {
  ...
  Future<void> updateScheduleCompleted({
    ...
    FeedingScheduleDatabase database =
        ref.watch(feedingScheduleDatabaseProvider);
    final newState = await AsyncValue.guard(() => database.updateDailySchedules(
        scheduleID, dailySchedule, userId, petId));
    if (mounted) {
      state = newState;
    }
    if (!state.hasError) {
